...
func TestGetJSON(t *testing.T) {
	ch := make(chan struct{})
	var c = client.NewAPIClient()
	var data = make(map[string]interface{})
	c.Get("http://"+ADDRESS+"/json").DoDecodeTo(&data, client.JSON, func(resp *http.Response, strct interface{}) {
...
func TestGetXML(t *testing.T) {
	...
	var c = client.NewAPIClient()
	var data = Data{}
	c.Get("http://"+ADDRESS+"/xml").DoDecodeTo(&data, client.XML, func(resp *http.Response, strct interface{}) {
		if resp.StatusCode != http.StatusOK {
			t.Error("Status code is not 200")
		}
		if data.Status != "ok" {
